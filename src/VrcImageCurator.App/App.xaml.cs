...
public partial class App : System.Windows.Application
{
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        {
            MessageBox.Show(exception.Message, "VRC Image Curator", MessageBoxButton.OK, MessageBoxImage.Error);
            Shutdown(2);
            return;
        }
        ...
        catch (Exception exception)
        {
            ...
            MessageBox.Show(
                exception.Message,
                "VRC Image Curator could not start",
                MessageBoxButton.OK,
    ...
    private void MainWindowClosing(object? sender, CancelEventArgs e)
    {
        ...
        e.Cancel = true;
        MainWindow?.Hide();
        _tray?.ShowNotification("VRC Image Curator", "Monitoring continues in the system tray.");
    }
        ...
        await Dispatcher.InvokeAsync(
            () =>
            {
                _tray?.ShowNotification(
                    ...
                    $"The {failure.Category} scan failed. Open VRC Image Curator for details.");
                if (MainWindow is MainWindow window)
                {
    ...
    private async Task ReportTrayScanFailureAsync(Exception exception)
    {
        ...
        await Dispatcher.InvokeAsync(
            () =>
            {
                _tray?.ShowNotification("Scan failed", "Open VRC Image Curator for details.");
                if (MainWindow is MainWindow window)
                {
    ...
    private async Task ReportBackgroundEventFailureAsync(string operation, Exception exception)
    {
        ...
        await Dispatcher.InvokeAsync(
            () =>
            {
                _tray?.ShowNotification("Background operation failed", "Open VRC Image Curator for details.");
                if (MainWindow is MainWindow window)
                {
